...
impl SchemaOutput {
    fn derivations(&self, current_system: &str) -> Vec<PathBuf> {
        ...
        for item in self.inventory.values() {
            determine_derivations(&mut derivations, item, current_system);
        }
    ...
    match item {
        ...
        InventoryItem::Parent(Parent { children }) => {
            for item in children.values() {
                determine_derivations(derivations, item, current_system);
            }
        }
    }
}

fn determine_systems(
    ...
    match item {
        ...
        InventoryItem::Parent(Parent { children }) => {
            for item in children.values() {
                determine_systems(systems, systems_without_runners, item, runner_map);
            }
        }
    }
}
